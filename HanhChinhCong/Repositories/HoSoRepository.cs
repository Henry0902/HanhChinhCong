...
public class HoSoRepository
{
    ...
    public List<HoSoInfo> GetHoSoPhanCongXuLy(int? userId, string searchMaHoSo, string searchName, string searchTenCongDan, string searchCMND_CCCD, int? searchIdTrangThai, int page, int pageSize, out int totalRows)
    {
        var result = new List<HoSoInfo>();
        totalRows = 0;
        using (var conn = new SqlConnection(connectionString))
